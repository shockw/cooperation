...
import com.reache.cooperation.modules.cms.dao.ArticleDao;
import com.reache.cooperation.modules.cms.dao.ArticleDataDao;
import com.reache.cooperation.modules.cms.dao.CategoryDao;
...
//		dao.updateDelFlag(id, isRe!=null&&isRe?Article.DEL_FLAG_NORMAL:Article.DEL_FLAG_DELETE);
	...
	public Page<Article> search(Page<Article> page, String q, String categoryId, String beginDate, String endDate){
		List<ArticleData> articleDatas = articleDataDao.findFullText(q);
		List<String> ids = new ArrayList<String>();
		if (articleDatas != null && articleDatas.size() > 0) {
			for (ArticleData articleData : articleDatas) {
				ids.add(articleData.getId());
			}
		}
		
		page.setCount(ids.size());
		
		int currentNum = page.getPageNo();
		int pageSize = page.getPageSize();
		List<Article> articles = new ArrayList<Article>();
		int count = 0;
		int start = (currentNum -1) * pageSize;
		int ent = currentNum * pageSize;
		for (String id : ids) {
			count ++;
			if(count > start && count <= ent){
				articles.add(articleDao.get(id));
			}
		}
