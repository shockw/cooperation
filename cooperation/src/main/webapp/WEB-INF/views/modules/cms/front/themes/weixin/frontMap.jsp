			...
			<c:forEach items="${fnc:getCategoryList(site.id, tpl.id, -1, '')}" var="category">
				<c:choose>
	    			<c:when test="${not empty category.href}">
		    			<c:choose>
			    			<c:when test="${fn:indexOf(category.href, '://') eq -1}"><c:set var="categoryurl" value="${ctx}${category.href}"/></c:when>
			    			<c:otherwise><c:set var="categoryurl" value="${category.href}"/></c:otherwise>
			    		</c:choose>
	    			</c:when>
	    			<c:otherwise><c:set var="categoryurl" value="${ctx}/list-${category.id}${urlSuffix}"/></c:otherwise>
	    		</c:choose>
